                                  ...
                                  const QRectF &rect)
{
    ...
    // کادر آبی مرز بوم طراحی (بخش «تکمیل ظاهری» - درخواست کاربر): دقیقاً مثل
    // نرم‌افزار واقعی پروتئوس، این کادر مشخص می‌کند چه بخشی از صفحه‌ی بی‌نهایتِ
    // Grid واقعاً «برگه‌ی طراحی» (sceneRect - همان ابعادی که کاربر موقع
    // «ایجاد پروژه جدید»، بخش ۱ مستند، انتخاب کرده) است؛ چیزی که این کادر
    // بیرونش باشد فقط فضای اضافه‌ی بوم است، نه بخشی از مدار نهایی.
    QPen sheetPen(QColor(30, 90, 200));
    sheetPen.setWidth(2);
    sheetPen.setCosmetic(true); // ضخامت خط با زوم عوض نشود - همیشه ۲ پیکسل واقعی
